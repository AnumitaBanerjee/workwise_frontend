		...
		addRfqProduct: (state, action) => {
			...
			let data = {
				index: action.payload.index
					? Math.max(...state.rfqProducts.map((item) => item.index)) + 1
					: state.rfqProducts.length + 1,
				product_id: action.payload.product_id,
				predefined_tds_file: action.payload.pd_tds_file_url
					? action.payload.pd_tds_file_url
					: "",
				predefined_qap_file: action.payload.pd_qap_file_url
					? action.payload.pd_qap_file_url
					: "",
			...
			if (true) {
				...
				data.vendors =
					action.payload?.vendors?.length > 0 ? action.payload?.vendors : [];
