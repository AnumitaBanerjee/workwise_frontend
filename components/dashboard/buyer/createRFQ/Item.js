...
const Item = ({ data, handleProductSpec, vendorApprovedList }) => {
	...
	const [uploadedQapFile, setuploadedQapFile] = useState("");
	const [uploadedSpecFile, setuploadedSpecFile] = useState("");
	...
	useEffect(() => {
		data.spec.map((item) => {
			if (item.title == "Size") {
				// console.log("Spec value: 47", item.value);
	...
	const handleSpecValue = (type, value) => {
		if (type == "size") {
			// console.log("Spec value: 71", value);
			setspecSize(value);
			dispatch(
				addProductSpecValue({
					title: "Size",
							...
							{data.predefined_tds_file != "" ? (
								<>
									<label>
										<input
											type="checkbox"
											checked={data.user_selected_predefined_tds}
											onClick={(e) =>
												handleSelectDefaultTDSQAPFile(e, "TDS", data)
											}
										/>{" "}
										Select file
									</label>
									<a
										href={data.predefined_tds_file}
							...
							{data.predefined_qap_file != "" ? (
								<>
									<label>
										<input
											type="checkbox"
											checked={data.user_selected_predefined_qap}
											onClick={(e) =>
												handleSelectDefaultTDSQAPFile(e, "QAP", data)
											}
										/>{" "}
										Select file
									</label>
